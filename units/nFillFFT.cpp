...
fill_fft_lookup [MAX_BLOCK_SIZE * 4];
...
double Apply_Window_Function(FFT_Proc_Rec* this_FFT_plan, double Filled_Average)
{
    ...
    for (int32_t ff_i = 0; ff_i < this_FFT_plan->FFT->length; ff_i++)
    {
        ...
        ff_l+= ff_m * ff_m;
        this_FFT_plan->DReal[ff_i] = ff_m * window_function[this_FFT_plan->FFT->bit_length][ff_i];
    }
...
double FillFFT_Input_From_WAVE(FFT_Proc_Rec* this_FFT_plan)
{
    ...
    for (int32_t ff_i = 0; ff_i<this_FFT_plan->FFT->length; ff_i++)
    {
        ...
        double ff_m = AudioData.WAVEPTR[fill_fft_lookup[ff_j].block][Global.Channel][fill_fft_lookup[ff_j].offset].Integers[0];
        ff_k+= ff_m;
        this_FFT_plan->DReal[ff_i] = ff_m;
    }

    double result = Apply_Window_Function(this_FFT_plan, ff_k);
    return result;
}
...
double FillFFT_Input_From_BTRD(FFT_Proc_Rec* this_FFT_plan)
{
    ...
    for (int32_t ff_i = 0; ff_i<this_FFT_plan->FFT->length; ff_i++)
    {
        ...
        if (ff_j < Global.Codec_Block.Size)
        {
            ...
            ff_m = AudioData.BTRDPTR[fill_fft_lookup[ff_j].block][Global.Channel][fill_fft_lookup[ff_j].offset].Integers[0] * settings.scaling_factor_inv;
        }
        else
            ff_m = AudioData.WAVEPTR[NEXT_CODEC_BLOCK][Global.Channel][ff_j - Global.Codec_Block.Size].Integers[0];
...
double FillFFT_Input_From_CORR(FFT_Proc_Rec* this_FFT_plan)
{
    ...
    for (int32_t ff_i = 0; ff_i<this_FFT_plan->FFT->length; ff_i++)
    {
        ...
        {
            ff_j += Global.Codec_Block.Size << 1;
            ff_m = AudioData.CORRPTR[fill_fft_lookup[ff_j].block][Global.Channel][fill_fft_lookup[ff_j].offset].Integers[0];
        }
        else
            ff_m = AudioData.WAVEPTR[NEXT_CODEC_BLOCK][Global.Channel][ff_j - Global.Codec_Block.Size].Integers[0];
...
void nFillFFT_Init()
{
    ...
    {
        window_function[nf_i] = new double[COMPLEX_SIZE << nf_i];
    }
    //=========================================================================================================================
